...
ak = 'qU0_z8uyB1v5NgtvCQL6-KgScCVzCTbzAd-pvtoR'
sk = 'ZIk9rMo4Y8RjmWx4QitGSuGasBQmjU3b3Ttdpt5W'
domain = 'cdn-md.littletrue.cn' # 上传域名
bucket = 'gxz-markdown' # 空间名称
...
def transfer_online_img(old_link):
    ...
    key = yyyymmdd+"_"+os.path.splitext(os.path.basename(sys.argv[1]) )[0]+"/image"+('{0}'.format(success+1)) + '.jpg'
    print(key)
...
if __name__ == '__main__':
    if len(sys.argv) >= 2:
        ...
        if os.path.isdir(sys.argv[1]):
            for file_path_arr in os.listdir(sys.argv[1]): 
                print('操作文件: ' + sys.argv[1] + '\\' + file_path_arr)
                # bak_md = '%s.bak' % (sys.argv[1] + '\\' + file_path_arr)
                # shutil.copyfile(sys.argv[1] + '\\' + file_path_arr, bak_md)  # 在执行改动之前备份原MD文件，可手动删除
                # print('origin markdown file backup in: %s' % (bak_md))
                find_md(sys.argv[1] + '\\' + file_path_arr)
